 ...
 */
export async function readParquetFromR2<T>(key: string): Promise<ParquetReadResult<T>> {
  try {
    // Fetch file from R2
    const result = await readFromBronze(key)
    if (!result.success || !result.data) {
      return { success: false, error: result.error || 'Failed to read from R2' }
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error reading Parquet',
    }
  }
}
...
export async function readBronzeFilesSince<T>(params: {
  ...
  dataType: DataType
  since: Date
}): Promise<{ success: boolean; records: T[]; files: string[]; error?: string }> {
  try {
    ...
    if (!listResult.success || !listResult.keys) {
      return { success: false, records: [], files: [], error: listResult.error }
    }
    ...
    for (const key of filteredKeys) {
      ...
      if (result.success && result.records) {
        ...
        console.warn(`[ParquetReader] Skipping file ${key}: ${result.error}`)
      }
    }
...
export async function readBronzeConversations(params: {
  tenantId: string
  since: Date
}): Promise<{ success: boolean; records: ConversationBronzeRecord[]; files: string[]; error?: string }> {
...
export async function readBronzeMessages(params: {
  tenantId: string
  since: Date
}): Promise<{ success: boolean; records: MessageBronzeRecord[]; files: string[]; error?: string }> {
...
export async function readBronzeVoiceCalls(params: {
  tenantId: string
  since: Date
}): Promise<{ success: boolean; records: VoiceCallBronzeRecord[]; files: string[]; error?: string }> {
...
export async function readBronzeSmsLogs(params: {
  tenantId: string
  since: Date
}): Promise<{ success: boolean; records: SmsLogBronzeRecord[]; files: string[]; error?: string }> {
 ...
 * Safe JSON parse
 */
export function safeJsonParse(value: string | null | undefined, fallback: unknown = {}): unknown {
