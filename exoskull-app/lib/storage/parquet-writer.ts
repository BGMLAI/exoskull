 ...
 */
export function conversationsToParquet(conversations: ConversationRecord[]): Uint8Array {
  ...
  const columnData = [
    ...
    { name: 'duration_seconds', data: conversations.map(c => c.duration_seconds || 0), type: 'INT32' as const },
...
export function messagesToParquet(messages: MessageRecord[]): Uint8Array {
  ...
  const columnData = [
    ...
    { name: 'duration_ms', data: messages.map(m => m.duration_ms || 0), type: 'INT32' as const },
    { name: 'audio_url', data: messages.map(m => m.audio_url || '') },
    { name: 'transcription_confidence', data: messages.map(m => m.transcription_confidence || 0), type: 'FLOAT' as const },
...
export function voiceCallsToParquet(calls: VoiceCallRecord[]): Uint8Array {
  ...
  const columnData = [
    ...
    { name: 'duration_seconds', data: calls.map(c => c.duration_seconds), type: 'INT32' as const },
 ...
 */
export function estimateParquetSize(records: unknown[], avgRecordBytes: number = 500): number {
 ...
 */
export function chunkRecords<T>(records: T[], targetSizeBytes: number = 50 * 1024 * 1024): T[][] {
  ...
  const recordsPerChunk = Math.max(1, Math.floor(targetSizeBytes / avgRecordBytes))
