...
import { ExoTool, ToolHandler, ToolResult, stringParam, numberParam } from './types';
...
export const taskTool: ExoTool = {
  name: 'task',
  description: 'Manage tasks across all connected sources (Google Tasks, Todoist, Notion, ExoSkull). Get tasks, create new ones, complete or update existing tasks.',
  parameters: {
    type: 'object',
    properties: {
      action: stringParam('Action to perform', {
        enum: ['get_tasks', 'create_task', 'complete_task', 'update_priority', 'get_overdue'],
      }),
...
const executor = new TaskManagerExecutor();

export const taskHandler: ToolHandler = async (context, params): Promise<ToolResult> => {
  const { action, title, description, due_date, priority, source, task_id, limit } = params as {
    action: string;
    title?: string;
    description?: string;
    due_date?: string;
  ...
  try {
    switch (action) {
      case 'get_tasks': {
        const data = await executor.getData(context.tenant_id);
      ...
      case 'create_task': {
        ...
        const result = await executor.executeAction(context.tenant_id, 'create_task', {
          title,
          description,
          due_date,
      ...
      case 'complete_task': {
        ...
        const result = await executor.executeAction(context.tenant_id, 'complete_task', {
          task_id,
          source: source || 'exoskull',
        });
      ...
      case 'update_priority': {
        ...
        const result = await executor.executeAction(context.tenant_id, 'update_priority', {
          task_id,
          priority,
          source: source || 'exoskull',
        });
