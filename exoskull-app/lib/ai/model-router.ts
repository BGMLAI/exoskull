...
export class ModelRouter {
  ...
  async route(options: AIRequestOptions): Promise<AIResponse> {
    ...
    if (options.forceModel) {
      return this.executeWithModel(options, options.forceModel, attemptedModels)
    }
    ...
    for (const model of pastSuccessfulModels) {
      if (circuitBreaker.isAllowed(model)) {
        try {
          const response = await this.executeWithModel(options, model, attemptedModels)
          return response
        } catch {
    ...
    return this.routeToTier(options, effectiveTier, attemptedModels, classification.category)
  }
  ...
  private async routeToTier(
    ...
    if (escalationCount < ROUTER_CONFIG.maxEscalations && tier < 4) {
      console.info(`[ModelRouter] Escalating from Tier ${tier} to Tier ${tier + 1}`)
    ...
    throw new AIRoutingError(
      `All models failed after ${escalationCount + 1} tier escalations`,
      attemptedModels
    )
  }
  ...
  private async executeWithModel(
    ...
    if (!provider) {
      throw new AIRoutingError(`No provider for model ${model}`, attemptedModels)
    }
    ...
    try {
      ...
      circuitBreaker.recordFailure(model, error instanceof Error ? error : undefined)
      throw error
    }
  }
   ...
   */
  private recordSuccess(tenantId: string | undefined, category: string, model: ModelId): void {
