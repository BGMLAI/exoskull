...
export class PermissionModel {
  ...
  private cache: Map<string, { grants: AutonomyGrant[]; timestamp: number }> = new Map()
  ...
  async checkPermission(
    ...
    try {
      ...
      if (error) {
        console.error('[PermissionModel] RPC error:', error)
      ...
      if (grant.dailyLimit && grant.useCount >= grant.dailyLimit) {
        const isToday = grant.lastUsedAt
          ? new Date(grant.lastUsedAt).toDateString() === new Date().toDateString()
          : false
        if (isToday) {
          return {
            granted: false,
      ...
      console.error('[PermissionModel] Check error:', error)
      return { granted: false, reason: 'no_matching_grant' }
    }
  }
  ...
  async checkPermissions(
    ...
    for (const action of actions) {
      ...
      if (!matchingGrant.isActive) {
        results.set(action, { granted: false, reason: 'disabled', grant: matchingGrant })
        continue
      }

      if (matchingGrant.expiresAt && new Date(matchingGrant.expiresAt) < new Date()) {
        results.set(action, { granted: false, reason: 'expired', grant: matchingGrant })
        continue
      }

      results.set(action, { granted: true, reason: 'granted', grant: matchingGrant })
    }
   ...
   */
  async getUserGrants(userId: string, useCache = true): Promise<AutonomyGrant[]> {
    // Check cache
    if (useCache) {
    ...
    if (error) {
      console.error('[PermissionModel] Get grants error:', error)
      return []
    }
  ...
  async createGrant(params: {
    ...
    if (error) {
      console.error('[PermissionModel] Create grant error:', error)
      return null
    }
  ...
  async revokeGrant(userId: string, grantId: string): Promise<boolean> {
    ...
    if (error) {
      console.error('[PermissionModel] Revoke grant error:', error)
      return false
    }
  ...
  async recordError(
    ...
    if (error) {
      console.error('[PermissionModel] Record error failed:', error)
    }
