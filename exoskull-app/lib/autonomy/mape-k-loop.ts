...
export class MAPEKLoop {
  ...
  async runCycle(
    ...
    console.log(`[MAPE-K] Starting cycle ${cycleId} for tenant ${tenantId} (trigger: ${trigger})`)
   ...
   */
  async analyze(tenantId: string, monitorData: MonitorData): Promise<AnalyzeResult> {
    ...
    if (monitorData.sleepHoursLast7d.length > 0) {
      const avgSleep = monitorData.sleepHoursLast7d.reduce((a, b) => a + b, 0) / monitorData.sleepHoursLast7d.length
      if (avgSleep < 6) {
        issues.push({
          type: 'sleep_debt',
        ...
        recommendations.push('Consider earlier bedtime or reducing screen time before sleep')
      }
    } else {
      gaps.push({
        area: 'health',
        ...
        lastMentioned: null,
        suggestedAction: 'Connect a sleep tracker (Oura, Apple Watch) or log sleep manually',
      })
    }
    ...
    if (monitorData.tasksOverdue > 5) {
      ...
      recommendations.push('Review and prioritize overdue tasks, consider delegating or rescheduling')
    }

    // 3. Analyze activity
    if (monitorData.activityMinutesLast7d.length > 0) {
      const avgActivity = monitorData.activityMinutesLast7d.reduce((a, b) => a + b, 0) / monitorData.activityMinutesLast7d.length
      if (avgActivity < 30) {
        issues.push({
          type: 'health_concern',
          severity: 'low',
    ...
    if (monitorData.conversationsLast24h === 0) {
      const lastInteraction = monitorData.lastInteractionAt
        ...
        : null
      if (!lastInteraction || Date.now() - lastInteraction.getTime() > 3 * 24 * 60 * 60 * 1000) {
        gaps.push({
          area: 'social',
    ...
    if (monitorData.connectedRigs.length === 0) {
      gaps.push({
        ...
        lastMentioned: null,
        suggestedAction: 'Connect Google Calendar, health trackers, or other services for richer insights',
      })
    }
   ...
   */
  async plan(tenantId: string, analyzeResult: AnalyzeResult): Promise<PlanResult> {
   ...
   */
  async execute(tenantId: string, planResult: PlanResult): Promise<ExecuteResult> {
    const errors: string[] = []
    ...
    for (const planned of planResult.interventions) {
      try {
        ...
        const { data: intervention, error: createError } = await this.supabase
          .rpc('propose_intervention', {
        ...
        if (!planned.requiresApproval) {
          ...
          await this.supabase
            ...
            .update({
              ...
              execution_result: actionResult.data ? { data: actionResult.data } : null,
          ...
          if (actionResult.success) {
            ...
            interventionsFailed++
            errors.push(`Intervention ${interventionId} failed: ${actionResult.error}`)
          }
   ...
   */
  async knowledge(tenantId: string, executeResult: ExecuteResult): Promise<KnowledgeResult> {
    ...
    if (recentFeedback && recentFeedback.length > 0) {
      for (const feedback of recentFeedback) {
        ...
        } else if (feedback.user_feedback === 'harmful' || feedback.user_feedback === 'unhelpful') {
          learnings.push(
