...
export class SleepTrackerExecutor implements IModExecutor {
  ...
  private ouraToSleepEntry(period: OuraSleepPeriod): SleepEntry {
    return {
      ...
      quality_score: period.efficiency
        ? Math.round(period.efficiency / 10)
        : null,
  ...
  private calculateStats(
    ...
    const avgQuality =
      qualityScores.length > 0
        ? qualityScores.reduce((sum, e) => sum + (e.quality_score || 0), 0) /
    ...
    const avgEfficiency =
      efficiencies.length > 0
        ? efficiencies.reduce((sum, e) => sum + (e.efficiency || 0), 0) /
          efficiencies.length
        : null;
  ...
  async getData(tenant_id: string): Promise<Record<string, unknown>> {
    ...
    if (ouraClient) {
      try {
        ...
        logger.error("[SleepTracker] Oura error:", error);
      }
    }
    ...
    entries = [...entries, ...uniqueManual].sort(
      (a, b) =>
        new Date(b.sleep_start).getTime() - new Date(a.sleep_start).getTime(),
    );
  ...
  async executeAction(
    ...
    try {
      switch (action) {
        case "log_sleep": {
          ...
          if (!sleep_start || !sleep_end) {
            return {
              success: false,
              error: "sleep_start and sleep_end are required",
            };
          }
        ...
        case "set_goal": {
          ...
          if (!target_hours || target_hours < 4 || target_hours > 12) {
            return {
              success: false,
              error: "target_hours must be between 4 and 12",
            };
          }
      ...
      logger.error("[SleepTracker] Action error:", {
        action,
        tenant_id,
