...
import {
  ...
  createNotionClient,
  NotionPage,
} from "../../rigs/notion/client";
import {
  TodoistClient,
  createTodoistClient,
  TodoistTask,
} from "../../rigs/todoist/client";
import {
  GoogleWorkspaceClient,
  createGoogleWorkspaceClient,
  GoogleTask,
} from "../../rigs/google-workspace/client";
...
export class TaskManagerExecutor implements IModExecutor {
  ...
  private async getRigConnection(
    tenantId: string,
    rigSlug: string,
  ...
  private notionToUnified(
    ...
    if (statusProp) {
      const statusValue =
        notionClient.getPropertyValue(statusProp)?.toString().toLowerCase() ||
        "";
      if (statusValue.includes("done") || statusValue.includes("complete")) {
        status = "done";
      } else if (
        statusValue.includes("progress") ||
        statusValue.includes("doing")
      ) {
        status = "in_progress";
      }
    }
    ...
    const priorityProp =
      page.properties["Priority"] || page.properties["priority"];
    if (priorityProp) {
      const priorityValue =
        notionClient.getPropertyValue(priorityProp)?.toString().toLowerCase() ||
        "";
      if (priorityValue.includes("urgent") || priorityValue.includes("p1")) {
        priority = "urgent";
      } else if (
        priorityValue.includes("high") ||
        priorityValue.includes("p2")
      ) {
        priority = "high";
      } else if (
        priorityValue.includes("low") ||
        priorityValue.includes("p4")
      ) {
        priority = "low";
      }
    }
  ...
  async getData(tenantId: string): Promise<Record<string, unknown>> {
    const [googleClient, notionClient, todoistClient, config] =
      await Promise.all([
    ...
    if (notionClient) {
      ...
      try {
        if (config.notion_database_id) {
          const notionPages = await notionClient.getAllDatabaseItems(
            config.notion_database_id,
          );
          tasks.push(
            ...notionPages.map((p) => this.notionToUnified(p, notionClient)),
          );
        } else {
          ...
          tasks.push(
            ...searchResult
              .slice(0, 50)
              .map((p) => this.notionToUnified(p, notionClient)),
          );
        }
      } catch (err) {
    ...
    // Fetch from ExoSkull internal tasks (via dual-read service)
    try {
      const exoTasks = await getTasksService(tenantId, { limit: 100 });
      if (exoTasks.length > 0) {
  ...
  async getInsights(tenantId: string): Promise<ModInsight[]> {
    ...
    if (stats.overdue > 0) {
      ...
      insights.push({
        ...
        message: `You have tasks past their due date: ${overdueTasks
          .slice(0, 3)
          .map((t) => t.title)
      ...
      switch (action) {
        case "create_task": {
          ...
          if (targetSource === "google") {
            ...
            return {
              success: true,
              result: { id: `google:${taskListId}:${task.id}`, task },
            };
          }
          ...
          if (targetSource === "todoist") {
            ...
            return {
              success: true,
              result: { id: `todoist:${task.id}`, task },
            };
          }
          ...
          if (targetSource === "notion") {
            ...
            const page = await client.createTask(
              config.notion_database_id,
              title,
              {
                priority,
          ...
          // Default: create in ExoSkull via dual-write service
          const priorityNumMap: Record<string, 1 | 2 | 3 | 4> = {
            urgent: 1,
            high: 2,
            medium: 3,
            low: 4,
          };
          const taskResult = await createTaskService(tenantId, {
            title,
            description: description || null,
            priority: priorityNumMap[priority || "medium"] || 3,
          ...
          return {
            success: true,
            result: {
              id: `exoskull:${taskResult.id}`,
              task: { id: taskResult.id, title },
            },
          };
        }
        ...
        case "complete_task": {
          ...
          if (source === "exoskull") {
            const [, id] = parts;
            const completeResult = await completeTaskService(id, tenantId);
            if (!completeResult.success)
              throw new Error(
                completeResult.error || "Failed to complete task",
              );
            return { success: true };
          }
        ...
        case "update_priority": {
          const { task_id, priority } = params as {
            task_id: string;
            priority: string;
          };
          ...
          if (source === "exoskull") {
            ...
            const updateResult = await updateTaskService(id, tenantId, {
              priority: priorityNumMap[priority] || 3,
            });
            if (!updateResult.success)
              throw new Error(
                updateResult.error || "Failed to update priority",
              );
            return { success: true };
          }
          ...
          return {
            success: false,
            error: "Priority update not supported for this source",
          };
        }
        ...
        case "sync": {
          ...
          return {
            success: true,
            result: { synced: (data.tasks as unknown[]).length },
          };
        }
    ...
    return [
      {
        ...
        name: "Create Task",
        description:
          "Create a new task in Google Tasks, Todoist, Notion, or ExoSkull",
        params_schema: {
          type: "object",
          required: ["title"],
          properties: {
            ...
            source: {
              type: "string",
              enum: ["google", "todoist", "notion", "exoskull"],
              default: "google",
            },
          },
        },
        ...
        params_schema: {
          ...
          properties: {
            task_id: {
              type: "string",
              description:
                "Task ID (format: source:id or source:listId:id for Google)",
            },
          },
        },
      },
      {
        ...
        params_schema: {
          ...
          properties: {
            task_id: {
              type: "string",
              description: "Task ID (format: source:id)",
            },
            priority: {
              type: "string",
              enum: ["low", "medium", "high", "urgent"],
