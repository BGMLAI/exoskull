...
export abstract class BaseAgent implements IAgent {
  ...
  async analyzeResources(tenantId: string): Promise<ResourceAnalysis> {
    ...
    return {
      availableData: {
        ...
        tasks: tasksCount,
        patterns: 0, // TODO: implement pattern counting
        mits: mitsCount,
      },
      connectedRigs: rigs,
      activeModules: [], // TODO: get from tenant config
      modelAvailability: {
        'gemini-flash': true,
        'claude-haiku': true,
        'kimi-k2': true,
      ...
      quotas: {
        aiCallsRemaining: 1000, // TODO: implement quota tracking
        storageUsedMb: 0,
        apiCallsToday: 0,
      },
    }
  ...
  async analyzeEnvironment(tenantId: string): Promise<EnvironmentAnalysis> {
    ...
    const [
      ...
      this.checkCalendarBusy(tenantId),
    ...
    return {
      ...
      lastConversationAgo: lastConversation
        ? Math.floor((now.getTime() - new Date(lastConversation).getTime()) / 60000)
        : -1,
      ...
      calendarBusy,
      upcomingEvents: 0, // TODO: implement calendar integration
  ...
  private async checkCalendarBusy(_tenantId: string): Promise<boolean> {
