...
class AgentRegistryImpl implements IAgentRegistry {
  ...
  register(agent: AgentRegistration): void {
    if (this.registrations.has(agent.id)) {
      console.warn(`[AgentRegistry] Agent ${agent.id} already registered, updating...`)
    }
  ...
  async spawn(request: SpawnRequest): Promise<IAgent | null> {
    ...
    console.warn(`[AgentRegistry] All matching agents unavailable for: ${request.task}`)
    this.failures++
    return null
  }
  ...
  findByTier(tier: AgentTier): AgentRegistration[] {
    return Array.from(this.registrations.values()).filter((r) => r.tier === tier)
  }
