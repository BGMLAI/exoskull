...
export class PatternLearnerAgent extends BaseAgent {
  ...
  async decide(
    ...
    if (resources.availableData.conversations < 20) {
      logger.info("[PatternLearner] Insufficient data for pattern analysis");
      return [];
    }
  ...
  private async detectPatterns(
    ...
    const timingPatterns = await this.analyzeConversationTiming(
      tenantId,
      windowDays,
    );
      ...
      if (existing) {
        ...
        await this.supabase
          ...
          .update({
            confidence: Math.min(
              1,
              (existing.confidence + pattern.confidence) / 2 + 0.1,
            ),
    ...
    if (sortedCategories.length > 0) {
      ...
      if (count >= 3) {
        ...
        patterns.push({
          ...
          description:
            categoryDescriptions[topCategory] || `Focus on ${topCategory}`,
          frequency: "irregular",
