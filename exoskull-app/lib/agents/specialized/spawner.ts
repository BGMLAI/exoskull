...
export interface SpawnTrigger {
  name: string
  description: string
  condition: (resources: ResourceAnalysis, environment: EnvironmentAnalysis) => boolean
...
const DEFAULT_SPAWN_TRIGGERS: SpawnTrigger[] = [
  ...
  },
  {
    ...
    condition: (_, env) => env.recentActivityLevel === 'none' && env.lastConversationAgo > 72 * 60,
    agentConfig: {
  ...
  },
  {
    ...
    condition: (res) => res.availableData.highlights < res.availableData.conversations * 0.1,
    agentConfig: {
      id: 'data-enricher',
      name: 'Data Enricher',
      tier: AGENT_TIERS.BALANCED,
...
export class SpawnerAgent extends BaseAgent {
  ...
  readonly capabilities = ['agent_spawning', 'dynamic_agents', 'need_detection']
  ...
  async decide(
    ...
    if (environment.isQuietHours) {
      console.log('[Spawner] Skipping - quiet hours')
      return []
    }
    ...
    for (const trigger of this.triggers) {
      try {
        ...
        console.error(`[Spawner] Error evaluating trigger ${trigger.name}:`, error)
      }
    }
  ...
  async execute(decision: Decision): Promise<ExecutionResult> {
    ...
    try {
      ...
      const existing = registry.findByCapability(agentConfig.capabilities?.[0] || '')
  ...
  private createDynamicAgent(
    ...
    console.log(`[Spawner] Registered dynamic agent: ${config.name}`)
    return registration
  }
...
class DynamicAgent extends BaseAgent {
  ...
  async execute(decision: Decision): Promise<ExecutionResult> {
    ...
    console.log(`[DynamicAgent:${this.name}] Executing ${decision.action}`)
...
export async function checkAndSpawnAgents(
  tenantId: string
): Promise<{
  success: boolean
  result?: unknown
  ...
  try {
    ...
    const results = await Promise.all(
      decisions.map((d) => spawner.execute(d))
    )
