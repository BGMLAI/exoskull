...
export class HighlightExtractorAgent extends BaseAgent {
  ...
  async decide(
    ...
    const hoursSinceLast = environment.lastHighlightExtraction
      ? (Date.now() - new Date(environment.lastHighlightExtraction).getTime()) /
        (1000 * 60 * 60)
      : Infinity;

    // Run every 15 minutes if there might be new conversations
    if (hoursSinceLast < 0.25 && environment.lastConversationAgo > 15) {
      logger.info("[HighlightExtractor] No new conversations to process");
      return [];
    }
  ...
  private async extractHighlights(
    ...
    const existingHighlights = await getUserHighlights(
      this.supabase,
      tenantId,
      100,
    );
    const existingContents = existingHighlights.map((h) =>
