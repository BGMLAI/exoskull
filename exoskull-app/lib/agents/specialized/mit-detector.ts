...
export class MITDetectorAgent extends BaseAgent {
  ...
  async decide(
    ...
    if (resources.availableData.conversations < 5) {
      logger.info("[MITDetector] Not enough conversations for analysis");
      return [];
    }

    // Check if MIT detection is due
    const hoursSinceLast = environment.lastMitDetection
      ? (Date.now() - new Date(environment.lastMitDetection).getTime()) /
        (1000 * 60 * 60)
      : Infinity;

    if (hoursSinceLast < 24 * 7) {
      // Weekly
      logger.info("[MITDetector] MIT detection not due yet");
      return [];
    }
  ...
  private async detectMITs(
    ...
    await this.supabase.rpc("log_learning_event", {
      ...
      p_data: {
        mits: mits.map((m) => ({ rank: m.rank, objective: m.objective })),
      },
      p_agent_id: this.id,
    });
    ...
    return clusteredGoals
      .map((goal) => {
        ...
        const daysSinceMention = Math.max(
          0,
          (Date.now() - new Date(goal.lastMentioned).getTime()) /
            (1000 * 60 * 60 * 24),
        );
