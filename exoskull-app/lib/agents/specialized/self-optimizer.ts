...
export class SelfOptimizerAgent extends BaseAgent {
  ...
  readonly capabilities = ['self_optimization', 'learning', 'performance_tuning', 'feedback_analysis']
  ...
  async decide(
    ...
    if (
      ...
      console.log('[SelfOptimizer] Skipping - not Sunday night and not forced')
      return []
    }

    // Need sufficient data to optimize
    if (resources.availableData.conversations < 10) {
      console.log('[SelfOptimizer] Skipping - insufficient data for optimization')
      return []
    }
    ...
    for (const stat of stats as any[]) {
      ...
      if (stat.total_helpful > stat.total_unhelpful * 2) {
        insights.push({
          ...
          confidence: 0.75,
          actionable: false,
          data: { type: stat.intervention_type, ratio: stat.total_helpful / Math.max(1, stat.total_unhelpful) },
        })
    ...
    if (conversations && conversations.length > 10) {
      ...
      if (peakCount > conversations.length * 0.2) {
        insights.push({
          ...
          data: { peakHour, peakCount, totalConversations: conversations.length },
        })
      }
      ...
      if (lateNightCount < conversations.length * 0.05) {
        insights.push({
          type: 'preference',
          description: 'User rarely interacts late night (11PM-6AM) - respect quiet hours',
          confidence: 0.8,
          actionable: false,
          data: { lateNightPercent: (lateNightCount / conversations.length * 100).toFixed(1) },
        })
      }
    }
  ...
  private async generateOptimizations(
    ...
    const allInsights = [...feedbackInsights, ...performanceInsights, ...patternInsights]
        ...
        case 'optimize_agent_speed':
          optimizations.push({
            target: {
              type: 'agent',
              targetId: target,
              metric: action === 'optimize_agent_tokens' ? 'response_time' : 'response_time',
              currentValue: 0,
              targetValue: 0.8,
            },
            action: 'enhance',
    ...
    for (const opt of optimizations) {
      ...
      if (opt.action === 'disable') {
        try {
          ...
          console.error('[SelfOptimizer] Failed to create optimization intervention:', error)
        }
      }
    }
  ...
  private async storeLearnings(
    ...
    const significantInsights = insights.filter(
      (i) => i.confidence > 0.7 && (i.type === 'pattern' || i.type === 'preference')
    )
    ...
    for (const insight of significantInsights) {
      try {
        await this.supabase.from('learning_events').insert({
          tenant_id: tenantId,
          event_type: insight.type === 'pattern' ? 'pattern_detected' : 'highlight_added',
          data: {
