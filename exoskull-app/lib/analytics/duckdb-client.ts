...
import { listBronzeFiles, readFromBronze, DataType } from '../storage/r2-client'
 ...
 */
export async function getBronzeFiles(params: BronzeQueryParams): Promise<string[]> {
...
export async function getBronzeSize(tenantId: string): Promise<{
  ...
  const dataTypes: DataType[] = ['conversations', 'messages', 'voice_calls', 'sms_logs']
  const result: {
    totalFiles: number
    totalBytes: number
    byType: Record<DataType, { files: number; bytes: number }>
...
export async function aggregateBronzeData<T extends { id: string }>(
  params: BronzeQueryParams & {
    aggregateBy: string
    metrics: Array<{ name: string; type: 'count' | 'sum' | 'avg' | 'min' | 'max'; field?: string }>
  }
): Promise<QueryResult<AggregationResult>> {
  const startTime = Date.now()
  const { tenantId, dataType, startDate, endDate, aggregateBy, metrics, limit } = params

  try {
    // Get relevant files
    const files = await getBronzeFiles({ tenantId, dataType, startDate, endDate })
    ...
    const allRecords: T[] = []
    for (const file of files.slice(0, limit ? Math.ceil(limit / 100) : files.length)) {
      ...
      if (result.success && result.data) {
        ...
        console.log(`[Analytics] Would read ${file}`)
      }
    }
    ...
    for (const record of allRecords) {
      const key = String((record as Record<string, unknown>)[aggregateBy] || 'unknown')
      if (!groups.has(key)) {
        groups.set(key, [])
      }
    ...
    for (const [groupKey, records] of groups) {
      ...
      for (const metric of metrics) {
        const values = records.map((r) =>
          metric.field ? Number((r as Record<string, unknown>)[metric.field]) || 0 : 1
        )
        ...
        switch (metric.type) {
          ...
          case 'sum':
            ...
            break
          case 'avg':
            row[metric.name] = values.length > 0 ? values.reduce((a, b) => a + b, 0) / values.length : 0
            break
          case 'min':
...
export function generateDuckDBSQL(query: AnalyticsQuery): string {
  ...
  if (orderBy && orderBy.length > 0) {
    const orders = orderBy.map((o) => `${o.field} ${o.direction.toUpperCase()}`)
...
export const ANALYTICS_QUERIES = {
  ...
  messagesByRole: (tenantId: string): AnalyticsQuery => ({
    select: ['role', 'COUNT(*) as message_count', 'AVG(duration_ms) as avg_duration_ms'],
  ...
  channelDistribution: (tenantId: string): AnalyticsQuery => ({
    select: ['channel', 'COUNT(*) as count', 'SUM(duration_seconds) as total_duration'],
    from: { tenantId, dataType: 'conversations' },
    groupBy: ['channel'],
