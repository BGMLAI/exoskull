...
export async function autoInstallMods(tenantId: string): Promise<string[]> {
  ...
  console.log('[ProactiveEngine] Mods to install for', tenantId, ':', Array.from(modSlugs))
  ...
  for (const mod of mods) {
    ...
    if (installError) {
      console.error(`[ProactiveEngine] Failed to install ${mod.slug}:`, installError)
    } else {
      installed.push(mod.slug)
    }
  }

  console.log(`[ProactiveEngine] Installed ${installed.length} mods for ${tenantId}:`, installed)
