...
const QUESTIONS: QuestionConfig[] = [
  ...
  },
  {
    ...
    type: 'textarea',
    placeholder: 'np. "Nie mogę się skupić", "Za dużo na głowie", "Nie śpię dobrze"...',
    icon: <Target className="w-6 h-6" />
  },
  {
...
export function OnboardingForm({ onComplete }: OnboardingFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('[OnboardingForm] Submit error:', error)
          ...
          <div className="grid grid-cols-1 gap-3">
            {question.options?.map(option => (
              ...
              >
                ...
                {value === option.value && <Check className="w-5 h-5 ml-auto text-blue-400" />}
              </button>
            ))}
          </div>
        )
            ...
            {question.options?.map(option => {
              ...
              return (
                ...
                >
                  ...
                  {selected && <Check className="w-4 h-4 ml-auto text-blue-400" />}
                </button>
              )
            })}
          </div>
    ...
    <Card className="w-full max-w-lg bg-slate-800/50 border-slate-700">
      <CardContent className="p-8">
        ...
        <div className="flex justify-between">
          ...
          {!question.required && question.type !== 'select' && question.type !== 'toggle' && (
            <Button
              onClick={nextStep}
              variant="ghost"
