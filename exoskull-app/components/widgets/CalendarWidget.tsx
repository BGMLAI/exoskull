    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        {upcomingItems.length === 0 ? (
          <p className="text-sm text-muted-foreground">Brak nadchodzacych wydarzen</p>
        ) : (
          upcomingItems.map((item) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {item.type === 'checkin' && <Clock className="h-4 w-4 text-amber-500" />}
                {item.type === 'custom' && <Calendar className="h-4 w-4 text-purple-500" />}
                <div>
