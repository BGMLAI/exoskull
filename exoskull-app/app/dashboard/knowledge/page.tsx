...
export default function KnowledgePage() {
  ...
  const [uploadStatuses, setUploadStatuses] = useState<Record<string, { status: 'pending' | 'uploading' | 'success' | 'error'; error?: string }>>({})
  ...
  }, [tenantId, notesSearch, selectedLoopSlug, selectedCampaignId, selectedQuestId, selectedOpId])
  ...
  async function loadLoops(tenant_id: string) {
    const res = await fetch(`/api/knowledge/loops?tenantId=${tenant_id}&withStats=true`)
    if (!res.ok) {
    ...
    if ((data.loops || []).length === 0) {
      ...
      const retry = await fetch(`/api/knowledge/loops?tenantId=${tenant_id}&withStats=true`)
  ...
  async function loadCampaigns(tenant_id: string) {
    const res = await fetch(`/api/knowledge/campaigns?tenantId=${tenant_id}&limit=200&offset=0`)
    if (!res.ok) return
    const data = await res.json()
  ...
  async function loadQuests(tenant_id: string) {
    const res = await fetch(`/api/knowledge/quests?tenantId=${tenant_id}&limit=200&offset=0`)
    if (!res.ok) return
    const data = await res.json()
  ...
  async function loadOps(tenant_id: string) {
    const res = await fetch(`/api/knowledge/ops?tenantId=${tenant_id}&limit=200&offset=0`)
    if (!res.ok) return
    const data = await res.json()
    setOps(data.ops || [])
  }
  ...
  const filteredLoops = useMemo(() => {
    ...
    return loops.filter((loop) =>
      [loop.name, loop.description, loop.slug].some((v) => v?.toLowerCase().includes(q))
    )
  }, [loops, treeSearch])

  const selectedLoop = loops.find((loop) => loop.slug === selectedLoopSlug) || null
  ...
  function toggleExpanded(list: string[], id: string, setList: (value: string[]) => void) {
  ...
  async function handleDeleteCampaign(campaign: Campaign) {
    ...
    await fetch(`/api/knowledge/campaigns?campaignId=${campaign.id}&tenantId=${tenantId}`, {
      method: 'DELETE',
    })
    await loadCampaigns(tenantId)
  }
  ...
  async function handleSaveQuest() {
    ...
    const payload = {
      ...
      tags: questForm.tags ? questForm.tags.split(',').map((t) => t.trim()).filter(Boolean) : [],
    }
  ...
  async function handleDeleteQuest(quest: Quest) {
    ...
    await fetch(`/api/knowledge/quests?questId=${quest.id}&tenantId=${tenantId}`, {
      method: 'DELETE',
    })
    await loadQuests(tenantId)
  }
  ...
  async function handleSaveOp() {
    ...
    const payload = {
      ...
      estimatedEffort: opForm.estimatedEffort ? parseInt(opForm.estimatedEffort, 10) : null,
      tags: opForm.tags ? opForm.tags.split(',').map((t) => t.trim()).filter(Boolean) : [],
    }
    ...
    for (const file of selectedFiles) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          nextStatuses[key] = { status: 'error', error: errorData?.error || 'Upload error' }
        } else {
  ...
  if (!tenantId) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-bold">Zaloguj się, aby zobaczyć Knowledge</h1>
      </div>
    )
  }
  ...
  return (
    <div className="p-8 space-y-6">
        ...
        <div className="flex-1 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Loops → Campaigns → Quests → Ops</p>
            </div>
          ...
          <div className="space-y-4">
            {filteredLoops.map((loop) => {
              const loopCampaigns = campaigns.filter((c) => c.loop_slug === loop.slug)
              ...
              return (
                <div key={loop.id} className="space-y-2">
                  <div className="flex items-start gap-2">
                    <button
                      className="mt-2"
                      onClick={() => toggleExpanded(expandedLoops, loop.id, setExpandedLoops)}
                    >
                      {isLoopExpanded ? (
                    ...
                    <div className="flex flex-col gap-2">
                      <Button size="icon" variant="ghost" onClick={() => openEditLoop(loop)}>
                        <FileText className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => handleDeleteLoop(loop)}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
                  ...
                  {isLoopExpanded && (
                    <div className="space-y-3 ml-8">
                      {loopCampaigns.map((campaign) => {
                        const campaignQuests = quests.filter((q) => q.campaign_id === campaign.id)
                        const isCampaignExpanded = expandedCampaigns.includes(campaign.id)
                        ...
                        return (
                          <div key={campaign.id} className="space-y-2">
                            <div className="flex items-start gap-2">
                              <button
                                className="mt-2"
                                onClick={() => toggleExpanded(expandedCampaigns, campaign.id, setExpandedCampaigns)}
                              >
                                {isCampaignExpanded ? (
                                  <ChevronDown className="h-4 w-4" />
                                ) : (
                            ...
                            {isCampaignExpanded && (
                              <div className="space-y-2 ml-8">
                                {campaignQuests.map((quest) => {
                                  const questOps = ops.filter((o) => o.quest_id === quest.id)
                                  const isQuestExpanded = expandedQuests.includes(quest.id)
                                  ...
                                  return (
                                    <div key={quest.id} className="space-y-2">
                                      <div className="flex items-start gap-2">
                                        <button
                                          className="mt-2"
                                          onClick={() => toggleExpanded(expandedQuests, quest.id, setExpandedQuests)}
                                        >
                                          {isQuestExpanded ? (
                                            <ChevronDown className="h-4 w-4" />
                                          ) : (
                                      ...
                                      {isQuestExpanded && (
                                        <div className="space-y-2 ml-8">
                                          ...
                                          {questOps.length === 0 && (
                                            <p className="text-xs text-muted-foreground">Brak ops</p>
                                          )}
                                        </div>
                                      )}
                                    </div>
                                ...
                                {campaignQuests.length === 0 && (
                                  <p className="text-xs text-muted-foreground">Brak questów</p>
                                )}
                              </div>
                            )}
                          </div>
                      ...
                      {loopQuests.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs text-muted-foreground">Questy bez kampanii</p>
                      ...
                      {loopOps.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs text-muted-foreground">Ops bez questu</p>
                          {loopOps.map((op) => (
                            <div key={op.id} className="flex items-start gap-2">
                              <div className="flex-1">
                                <OpCard
                  ...
                  <div key={note.id} className="border rounded-md p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => handleDeleteNote(note)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
      ...
      <Dialog open={loopDialogOpen} onOpenChange={setLoopDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingLoop ? 'Edytuj Loop' : 'Nowy Loop'}</DialogTitle>
            <DialogDescription>Loop to główna kategoria życia.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <div className="space-y-2 flex-1">
                ...
                <Input
                  value={loopForm.icon}
                  onChange={(e) => setLoopForm((prev) => ({ ...prev, icon: e.target.value }))}
                />
              </div>
              <div className="space-y-2 flex-1">
                <Label>Kolor</Label>
                <Input
                  ...
                  onChange={(e) => setLoopForm((prev) => ({ ...prev, color: e.target.value }))}
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setLoopForm((prev) => ({ ...prev, priority: parseInt(e.target.value, 10) }))}
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setLoopDialogOpen(false)}>Anuluj</Button>
      ...
      <Dialog open={campaignDialogOpen} onOpenChange={setCampaignDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingCampaign ? 'Edytuj Campaign' : 'Nowa Campaign'}</DialogTitle>
            <DialogDescription>Campaign to inicjatywa w obrębie loopa.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <Label>Tytuł</Label>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => setCampaignForm((prev) => ({ ...prev, vision: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label>Loop</Label>
              <Select
                ...
                onValueChange={(value) => setCampaignForm((prev) => ({ ...prev, loopSlug: value === 'none' ? '' : value }))}
              >
                <SelectTrigger>
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-2 flex-1">
                ...
                <Input
                  ...
                  onChange={(e) => setCampaignForm((prev) => ({ ...prev, targetDate: e.target.value }))}
                />
              </div>
            </div>
            {editingCampaign && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCampaignDialogOpen(false)}>Anuluj</Button>
      ...
      <Dialog open={questDialogOpen} onOpenChange={setQuestDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingQuest ? 'Edytuj Quest' : 'Nowy Quest'}</DialogTitle>
            <DialogDescription>Quest to projekt w ramach kampanii.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <Label>Tytuł</Label>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setQuestForm((prev) => ({ ...prev, campaignId: value === 'none' ? '' : value }))}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setQuestForm((prev) => ({ ...prev, loopSlug: value === 'none' ? '' : value }))}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Wybierz loop" />
                </SelectTrigger>
            ...
            <div className="flex gap-3">
              <div className="space-y-2 flex-1">
                ...
                <Input
                  ...
                  onChange={(e) => setQuestForm((prev) => ({ ...prev, targetOps: e.target.value }))}
                />
              </div>
              <div className="space-y-2 flex-1">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setQuestForm((prev) => ({ ...prev, tags: e.target.value }))}
              />
            </div>
            {editingQuest && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setQuestForm((prev) => ({ ...prev, status: value }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Status" />
                  </SelectTrigger>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setQuestDialogOpen(false)}>Anuluj</Button>
      ...
      <Dialog open={opDialogOpen} onOpenChange={setOpDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Op to konkretne zadanie w ramach questu.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <Label>Tytuł</Label>
              <Input
                value={opForm.title}
                onChange={(e) => setOpForm((prev) => ({ ...prev, title: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label>Opis</Label>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setOpForm((prev) => ({ ...prev, questId: value === 'none' ? '' : value }))}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setOpForm((prev) => ({ ...prev, loopSlug: value === 'none' ? '' : value }))}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Wybierz loop" />
                </SelectTrigger>
            ...
            <div className="flex gap-3">
              <div className="space-y-2 flex-1">
                ...
                <Input
                  ...
                  onChange={(e) => setOpForm((prev) => ({ ...prev, priority: e.target.value }))}
                />
              </div>
              <div className="space-y-2 flex-1">
                ...
                <Input
                  ...
                  onChange={(e) => setOpForm((prev) => ({ ...prev, dueDate: e.target.value }))}
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setOpForm((prev) => ({ ...prev, scheduledFor: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setOpForm((prev) => ({ ...prev, estimatedEffort: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label>Tagi (oddzielone przecinkami)</Label>
              <Input
                value={opForm.tags}
                onChange={(e) => setOpForm((prev) => ({ ...prev, tags: e.target.value }))}
              />
            </div>
            {editingOp && (
              <div className="space-y-2">
                ...
                <Select
                  value={opForm.status}
                  onValueChange={(value) => setOpForm((prev) => ({ ...prev, status: value }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Status" />
                  </SelectTrigger>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpDialogOpen(false)}>Anuluj</Button>
      ...
      <Dialog open={noteDialogOpen} onOpenChange={setNoteDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingNote ? 'Edytuj Note' : 'Nowa Note'}</DialogTitle>
            <DialogDescription>Twórz notatki i przypisuj kontekst.</DialogDescription>
          </DialogHeader>
          <NoteEditor
            ...
            initial={{
              ...
              loopSlug: editingNote?.loop_slug || selectedLoopSlug || selectedQuest?.loop_slug || selectedCampaign?.loop_slug || null,
