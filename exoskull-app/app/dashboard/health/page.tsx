...
export default async function HealthDashboardPage() {
  ...
  if (!user) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-bold">Zaloguj sie, aby zobaczyc dane zdrowotne</h1>
      </div>
    )
  }
    ...
    <div className="p-8 space-y-6">
      ...
      {!isConnected && (
        <Card className="border-amber-500/50 bg-amber-500/5">
          <CardHeader>
            ...
            <CardDescription>
              Zainstaluj aplikacje ExoSkull na Androidzie, aby synchronizowac dane zdrowotne
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="p-3 rounded-lg bg-muted/50">
              <p className="text-xs text-muted-foreground">
                <strong>Tymczasowo:</strong> Mozesz testowac API wysylajac dane manualnie przez endpoint{' '}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          label="Sen"
          value={metricsByType.sleep ? Math.round(metricsByType.sleep.value / 60) : 0}
          unit="h"
          subValue={metricsByType.sleep ? `${metricsByType.sleep.value % 60}min` : undefined}
          ...
          {avgSteps > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 rounded-lg bg-muted/50">
                ...
                <p className="text-sm text-muted-foreground">Srednia krokow/dzien</p>
              </div>
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <p className="text-2xl font-bold">{Object.keys(stepsByDay).length}</p>
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Polacz Health Connect, aby zaczac sledzic</p>
            </div>
          )}
        </CardContent>
      </Card>
...
function MetricCard({ icon, label, value, unit, subValue, color, hasData }: MetricCardProps) {
  return (
    <Card>
