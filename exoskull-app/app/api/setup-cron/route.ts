  ...
  try {
    ...
    if (checkJobsError && checkJobsError.code === '42P01') {
      ...
      return NextResponse.json({
        success: false,
        message: 'Tables not created yet. Please run the SQL migration in Supabase Dashboard.',
        instructions: [
        ...
        migration_file: 'supabase/migrations/20260201000002_scheduled_jobs_system.sql'
      }, { status: 400 })
    }
    ...
    const defaultJobs = [
      {
        ...
        description: 'Daily morning wellness check: "Jak się czujesz? Energia 1-10?"',
        cron_expression: '0 * * * *',
    ...
    for (const job of defaultJobs) {
      ...
      if (error) {
        results.push({ step: `insert_job_${job.job_name}`, status: 'error', error: error.message })
      } else {
...
export async function GET() {
  try {
    ...
    const { data: jobs, error: jobsError } = await supabase
      .from('exo_scheduled_jobs')
      .select('job_name, display_name, time_window_start, default_channel, is_active')
      .order('time_window_start')
    ...
    const { data: functions } = await supabase.rpc('get_users_for_scheduled_job', {
