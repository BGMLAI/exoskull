...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: users, error } = await supabase
      ...
      .select('user_id, enabled_checks, impulse_interval_minutes, last_impulse_at')
      .or(`last_impulse_at.is.null,last_impulse_at.lt.${new Date(Date.now() - 30 * 60 * 1000).toISOString()}`)
    ...
    for (const user of users || []) {
      try {
        const result = await runPulseForUser(user.user_id, user.enabled_checks || ['health', 'tasks', 'calendar'])
...
async function runPulseForUser(userId: string, enabledChecks: CheckType[]): Promise<PulseResult> {
...
async function runCheck(userId: string, check: CheckType, connectedRigs: string[]): Promise<PulseAlert[]> {
  switch (check) {
    case 'health':
...
async function runHealthCheck(userId: string, connectedRigs: string[]): Promise<PulseAlert[]> {
  const alerts: PulseAlert[] = []
...
async function runTasksCheck(userId: string): Promise<PulseAlert[]> {
  ...
  if (overdueCount && overdueCount > 0) {
    const highPriority = overdueTasks?.filter((t) => t.priority === 'high').length || 0
...
async function runCalendarCheck(userId: string, connectedRigs: string[]): Promise<PulseAlert[]> {
  const alerts: PulseAlert[] = []
...
async function runFinanceCheck(userId: string, connectedRigs: string[]): Promise<PulseAlert[]> {
  const alerts: PulseAlert[] = []
  ...
  if (recentSpending && avgSpending && avgSpending.length > 0) {
    const weekTotal = recentSpending.reduce((sum, t) => sum + Math.abs(t.amount as number), 0)
    const monthTotal = avgSpending.reduce((sum, t) => sum + Math.abs(t.amount as number), 0)
    const weeklyAvg = monthTotal / 4

    if (weekTotal > weeklyAvg * 1.2) {
      const overspendPercent = Math.round(((weekTotal - weeklyAvg) / weeklyAvg) * 100)
      alerts.push({
        type: 'overspending',
