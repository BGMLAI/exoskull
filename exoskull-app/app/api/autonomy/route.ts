...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidPattern(actionPattern)) {
      return NextResponse.json(
        { error: 'Invalid action pattern. Use format: action or action:scope or action:*' },
        { status: 400 }
      )
    }
    ...
    console.log(`[Autonomy] Grant created: ${actionPattern} for user ${userId}`)
...
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json()
    const { grantId, userId, isActive, spendingLimit, dailyLimit, expiresAt } = body
