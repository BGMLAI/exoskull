  ...
  try {
    ...
    if (!operation) {
      return NextResponse.json(
        {
          error:
            "operation required (run_cycle, execute_action, approve, reject, feedback, detect_gaps, optimize, spawn_agents)",
        },
        { status: 400 },
      );
    }
...
async function handleFeedback(
  ...
  if (!["helpful", "neutral", "unhelpful", "harmful"].includes(feedback)) {
    throw new Error(
      "feedback must be: helpful, neutral, unhelpful, or harmful",
    );
  }
