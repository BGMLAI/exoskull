  ...
  try {
    ...
    if (!isGranted) {
      ...
      if (grant) {
        ...
        else if (grant.expires_at && new Date(grant.expires_at) < new Date()) reason = 'expired'
        else if (grant.daily_limit && grant.use_count >= grant.daily_limit) reason = 'daily_limit_reached'
      } else {
        reason = 'no_matching_grant'
      }
    }

    console.log(`[Autonomy Check] ${action} for ${userId}: ${isGranted ? 'GRANTED' : reason}`)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.log(`[Autonomy Check] Error recorded for ${action}: ${errorMessage}`)
