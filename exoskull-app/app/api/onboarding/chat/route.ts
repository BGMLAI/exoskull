...
export async function POST(request: NextRequest) {
  try {
    ...
    const isClosing = lowerMessage.includes('dziękuję') ||
...
###END_PROFILE_DATA###`
    ...
    let reply = openaiResult.choices?.[0]?.message?.content || 'Przepraszam, nie zrozumiałem.'

    // Check for profile data in response
    let shouldComplete = false
    let profileData = null

    const profileMatch = reply.match(/###PROFILE_DATA###\s*([\s\S]*?)\s*###END_PROFILE_DATA###/)
    if (profileMatch) {
      try {
        profileData = JSON.parse(profileMatch[1])
        shouldComplete = true
        // Remove the JSON from the reply
        reply = reply.replace(/###PROFILE_DATA###[\s\S]*###END_PROFILE_DATA###/, '').trim()
      } catch (e) {
