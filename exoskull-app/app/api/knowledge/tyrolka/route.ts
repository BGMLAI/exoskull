...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: dbContext, error: rpcError } = await supabase.rpc('get_tyrolka_context', {
...
async function assembleTyrolkaContext(tenantId: string): Promise<TyrolkaContext> {
  const now = new Date()
  ...
  const context: TyrolkaContext = {
    ...
    activeContext: {
      ...
      quests: (questsResult.data || []).map((q) => ({
        title: q.title,
        progress: q.target_ops ? Math.round((q.completed_ops / q.target_ops) * 100) : 0,
      })),
...
function synthesize(
  ...
  const topHighlights = highlights.slice(0, 3).map((h) => h.content).join('; ')
  const currentState = topHighlights || 'Brak wystarczających danych o użytkowniku'
  ...
  for (const mit of mits) {
    const hasRelatedOps = ops.some(
      (op) =>
        op.title.toLowerCase().includes(mit.objective.toLowerCase().split(' ')[0]) ||
        mit.objective.toLowerCase().includes(op.title.toLowerCase().split(' ')[0])
    )
  ...
  // Suggested focus
  const overdueOps = ops.filter((o) => o.due_date && new Date(o.due_date) < new Date())
...
function formatForPrompt(context: TyrolkaContext | Record<string, unknown>): string {
  const parts: string[] = []

  // Handle both formats (DB function result vs assembled)
  const ja = (context as TyrolkaContext).ja || (context as Record<string, unknown>).ja
  const nieJa = (context as TyrolkaContext).nieJa || (context as Record<string, unknown>).nieJa
  const objectives = (context as TyrolkaContext).objectives || (context as Record<string, unknown>).objectives
  ...
  // NIE-JA section
  if (Array.isArray((nieJa as TyrolkaContext['nieJa'])?.research) && (nieJa as TyrolkaContext['nieJa']).research.length > 0) {
  ...
  if (Array.isArray((activeContext as TyrolkaContext['activeContext'])?.ops) &&
      (activeContext as TyrolkaContext['activeContext']).ops.length > 0) {
    parts.push('\n## Aktywne zadania')
    for (const op of (activeContext as TyrolkaContext['activeContext']).ops.slice(0, 3)) {
      const dueStr = op.dueDate ? ` (do: ${new Date(op.dueDate).toLocaleDateString('pl-PL')})` : ''
